...
int main(){
	...
	do {
	 ...
	 if(yes_no(current->question)){
		...
		printf("Give me question that is TRUE for %s but not for %s? ",
			suspect, current->question);
		fgets(question, 80, stdin);
	//	free(current->question); //Ao descomentar esta linha o problema de vazamento de memória será corrigido
